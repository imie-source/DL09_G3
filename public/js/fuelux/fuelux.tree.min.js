(function (a, c) {
    var b = function (e, d) {
        ...
        this.$element.on("click", ".tree-folder-header", a.proxy(function (f) {
            if(f.target.id != 'folder-selector'){
            ...
            this.options.dataSource.data(f.data(), function (g) {
                ...
                a.each(g.data, function (h, j) {
                    ...
                    if (j.type === "folder") {
                        ...
                        i.find(".tree-folder-name").before('<span class="tree-item" data-id="' + j.id + '" data-name="' + j.name + '"><i id="folder-selector" class="icon-remove"></i></span>');
                        if(j.children > 0){
                            i.find(".tree-folder-name").after(' <span class="badge badge-grey" id="badge-nb-sub-cat">' + j.children + '</span>');
                        }
                        i.find(".tree-folder-name").after(' <a class="btn-skill-remover btn btn-minier btn-danger pull-right hidden" href="/skills/delete/' + j.id + '">&times;</a>');
                        k.data(j);
                        if ("icon-class" in j) {
                        ...
                        if (j.type === "item") {
                            ...
                            i.attr('data-id', j.id);
                            i.attr('data-name', j.name);
                            i.find(".tree-item-name").html(j.name);
                            i.find(".tree-item-name").after(' <a class="btn-skill-remover btn btn-minier btn-danger pull-right hidden" href="/skills/delete/' + j.id + '">&times;</a>');
                            i.data(j);
